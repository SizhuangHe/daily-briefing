...
A personal daily briefing app that keeps me updated on what's happening in the world and in my life. It aggregates news from multiple RSS sources, enriches them with AI (Gemini), and presents a structured morning briefing alongside calendar events and daily inspiration.

## Tech Stack

- **Backend:** Python, FastAPI, SQLAlchemy, SQLite
- **Frontend:** React 19, TypeScript, Vite, Tailwind CSS v4, TanStack React Query
- **AI:** Google Gemini API (article summarization, story clustering, importance scoring, topic/region classification, text embeddings)
...
**Today at a Glance** — A domain-based overview generated by Gemini that summarizes today's news landscape in 3-5 thematic domains (e.g., "Geopolitics", "Technology", "Markets"), each with a 1-2 sentence summary. Displayed at the top of the briefing for quick context.
...
**Caching:** The briefing is cached server-side with smart regeneration logic — it won't re-run the full Gemini pipeline unless enough new articles have arrived (≥5) or the cache has aged past 2 hours. A `force` parameter allows manual refresh.
...
Article text (`title + description`) is embedded using **Gemini `gemini-embedding-001`**. Embeddings are cached in the `Article.embedding` column (JSON-encoded float vectors) and only generated once per article.
...
A browsable library of all articles with filtering by topic, source, and geographic region. Articles are sorted by recommendation score or time. Each article shows topic tags, region tags, source, and age.

### Geographic Region Tagging

Articles are automatically classified by geographic region using Gemini (e.g., `us`, `china`, `europe`, `middle_east`). Region preferences are learned from user ratings and visible in the Dev dashboard.

### Data Retention

Articles older than 10 days are automatically pruned on every news refresh to keep the database lean and relevant.
...
- **Settings:** Topic interest selection, RSS source management
- **Dev Dashboard:** User profile (topic weights, content profile centroid, region preferences), evaluation metrics, and system diagnostics
...
backend/
  app/
    routers/          # FastAPI endpoints (news, briefing, calendar, preferences, inspiration, dev)
    ...
    services/         # Business logic
      ...
      gemini_service.py      # Gemini API calls (summarization, topic/region classification, embeddings)
      news_service.py        # RSS feed fetching, dedup, auto-pruning
...
frontend/
  src/
    components/
      ...
      News/           # News Library — article cards, ratings, topic/region filter
      Inspiration/    # Daily quote/fact/activity
      Settings/       # Topic selector, source manager
      Dev/            # Dev dashboard — user profile, evaluation metrics
